 ...
 */
export function getLlmInstance(role: 'default' | 'fast' = 'default'): LanguageModel {
  ...
  const getModelName = (providerName: string): string => {
    ...
    if (!modelName) {
      throw new Error(`環境変数 ${modelEnvVar} が.envファイルに設定されていません。`);
    }
    return modelName;
  };
  ...
  switch (provider) {
    case "google": {
      ...
      if (!apiKey) throw new Error("GOOGLE_API_KEYが.envファイルに設定されていません。");
    ...
    case "groq": {
      ...
      if (!apiKey) throw new Error("GROQ_API_KEYが.envファイルに設定されていません。");
    ...
    case "openrouter": {
      ...
      if (!apiKey) throw new Error("OPENROUTER_API_KEYが.envファイルに設定されていません。");
      const openrouter = createOpenAI({
        apiKey: apiKey,
        baseURL: "https://openrouter.ai/api/v1",
        headers: {
          "HTTP-Referer": process.env.OPENROUTER_HTTP_REFERER || "http://localhost:3000",
          "X-Title": process.env.OPENROUTER_X_TITLE || "Stagehand Agent Console",
        },
      });
    ...
    case "cerebras": {
      ...
      if (!apiKey) throw new Error("CEREBRAS_API_KEYが.envファイルに設定されていません。");
      // Cerebras APIはOpenAI互換のため、createOpenAIクライアントを使用する
      const cerebras = createOpenAI({
        apiKey: apiKey,
        baseURL: "https://api.cerebras.ai/v1", // Cerebrasの公式APIエンドポイント
      });
      const modelName = getModelName("cerebras");
      return cerebras(modelName);
    }
    default:
      throw new Error(`サポートされていないLLMプロバイダです: "${provider}"。'google', 'groq', 'openrouter', 'cerebras'のいずれかを指定してください。`);
  }
}
