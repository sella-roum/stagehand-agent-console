 ...
 * @param error - 直前のステップで発生したエラーメッセージ（あれば）
 * @returns - AIへの指示として整形されたプロンプト文字列
 */
function buildPrompt(task: string, url: string, summary: string, history: any[], error?: string): string {
...
${error ? `# 注意: 直前のステップで以下のエラーが発生しました。これを考慮して計画を修正してください。\nエラー: ${error}` : ''}
...
export async function taskAutomationAgent(task: string, page: Page) {
  ...
  while (loopCount < maxLoops) {
    ...
    const prompt = buildPrompt(task, page.url(), currentSummary, executionHistory);
    ...
    try {
        ...
        // ループの次のイテレーションで、このエラー情報を基に新しい計画が立て直される
    }
