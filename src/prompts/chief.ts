...
3.  **成功条件の定義:** 各サブゴールに対して、それが完了したと客観的に判断できる「成功条件」を必ず定義してください。
    - 良い例: 「ページのタイトルが'ダッシュボード'になっている」「'ログイン成功'というテキストが表示される」
    - 悪い例: 「ログインが成功する」「情報を取得する」
4.  **ツールの意識:** 実行エージェントが利用できるツール（ブラウザ操作、ファイル操作など）を念頭に置き、それらで達成可能な粒度のタスクにしてください。
5.  **アクション指向:** 各サブゴールは、エージェントが実行すべき具体的な「行動」を記述するようにしてください。「情報を確認する」のような曖昧なものではなく、「ページからテキストを抽出する」「ボタンをクリックする」といった形にします。エージェントが既に知っている情報（例：現在のURL）を取得するだけのステップは不要です。
6.  **出力の厳守:** 必ず指定されたJSONスキーマで出力してください。
...
2.  **再計画:** 上記の分析に基づき、**残りのタスクを達成するための新しいサブゴールリスト**を生成してください。各サブゴールには客観的な「成功条件」を必ず含めてください。
